...
export async function generateProject(o: StackOptions): Promise<void> {
  ...
  if (o.git) { try { await command('git', ['init'], o.targetDir); await command('git', ['add', '.'], o.targetDir); await command('git', ['commit', '-m', 'chore: initialize project with StackForge'], o.targetDir); } catch { /* Git identity is optional. */ } }
  if (o.install) { const [cmd, args] = installCommand(o.packageManager); await command(cmd, args, o.targetDir); if (o.backend === 'go') await command('go', ['mod', 'tidy'], `${o.targetDir}/apps/api`); if (o.backend === 'nest' && o.database !== 'none') await command(o.packageManager, o.packageManager === 'npm' ? ['run', 'prisma:generate', '-w', '@stackforge/api'] : ['--filter', '@stackforge/api', 'prisma:generate'], o.targetDir); }
}
