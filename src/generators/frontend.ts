...
async function vite(o: StackOptions, app: Exclude<AppName, 'api'>): Promise<void> {
  ...
  await json(root, `${base}/package.json`, { name: `@stackbuild/${app}`, private: true, version: '0.1.0', type: 'module', scripts: { dev: `vite --port ${VITE_PORTS[app]}`, build: 'tsc -b && vite build', lint: 'eslint .', typecheck: 'tsc -b --noEmit' }, dependencies: viteDeps(o), devDependencies: { '@types/node': 'latest', '@types/react': 'latest', '@types/react-dom': 'latest', '@vitejs/plugin-react': STACK_VERSIONS.viteReact, typescript: STACK_VERSIONS.typescript, vite: STACK_VERSIONS.vite, eslint: 'latest' } });
...
async function next(o: StackOptions, app: Exclude<AppName, 'api'>): Promise<void> {
  const root = o.targetDir, base = `apps/${app}`;
  await json(root, `${base}/package.json`, { name: `@stackbuild/${app}`, private: true, version: '0.1.0', scripts: { dev: `next dev --port ${NEXT_PORTS[app]}`, build: 'next build', start: `next start --port ${NEXT_PORTS[app]}`, lint: 'eslint .', typecheck: 'tsc --noEmit' }, dependencies: nextDeps(o), devDependencies: { '@types/node': 'latest', '@types/react': 'latest', '@types/react-dom': 'latest', typescript: STACK_VERSIONS.typescript, eslint: 'latest' } });
...
function viteDeps(o: StackOptions) { return { react: STACK_VERSIONS.react, 'react-dom': STACK_VERSIONS.react, 'react-router-dom': 'latest', axios: 'latest', '@tanstack/react-query': 'latest', zustand: 'latest', ...(o.ui === 'mui' ? { '@mui/material': 'latest', '@mui/icons-material': 'latest', '@emotion/react': 'latest', '@emotion/styled': 'latest' } : o.ui === 'tailwind' ? { tailwindcss: 'latest' } : {}) }; }
function nextDeps(o: StackOptions) { return { next: STACK_VERSIONS.next, react: STACK_VERSIONS.react, 'react-dom': STACK_VERSIONS.react, axios: 'latest', '@tanstack/react-query': 'latest', zustand: 'latest', ...(o.ui === 'mui' ? { '@mui/material': 'latest', '@mui/icons-material': 'latest', '@emotion/react': 'latest', '@emotion/styled': 'latest' } : o.ui === 'tailwind' ? { tailwindcss: 'latest', '@tailwindcss/postcss': 'latest' } : {}) }; }
