...
export async function generateProject(o: StackOptions): Promise<void> {
  try {
    await fs.ensureDir(o.targetDir);
    await generateRoot(o);
    await generatePackages(o);
    for (const app of o.apps.filter((x) => FRONTEND_APPS.includes(x)))
      await generateFrontend(o, app as "customer" | "admin" | "partner");
    await generateBackend(o);
    if (o.backend === "go") {
      await command("go", ["mod", "tidy"], `${o.targetDir}/apps/api`, {
        GOCACHE: path.join(os.tmpdir(), "stackbuild-go-build-cache"),
      });
    ...
    if (o.git) {
      try {
        await command("git", ["init"], o.targetDir);
        await command("git", ["add", "."], o.targetDir);
        await command(
          "git",
          ["commit", "-m", "chore: initialize project with StackBuild"],
          o.targetDir
        );
      } catch {
    ...
    if (o.install) {
      ...
      const packageManagerEnv =
        o.packageManager === "pnpm"
          ? { COREPACK_ENABLE_DOWNLOAD_PROMPT: "0" }
          : undefined;
      await command(cmd, args, o.targetDir, packageManagerEnv);
      if (o.backend === "nest" && o.database !== "none")
        await command(
          o.packageManager,
          o.packageManager === "npm"
            ? ["run", "prisma:generate", "-w", "@stackbuild/api"]
            : o.packageManager === "yarn"
              ? ["workspace", "@stackbuild/api", "prisma:generate"]
              : ["--filter", "@stackbuild/api", "prisma:generate"],
          o.targetDir,
          packageManagerEnv
        );
    }
    ...
    await fs.outputFile(
      `${o.targetDir}/STACKBUILD_RECOVERY.md`,
    ...
    throw new Error(
      `Project files were created in ${o.targetDir}, but setup did not finish. See STACKBUILD_RECOVERY.md for retry steps.\n${error instanceof Error ? error.message : String(error)}`
    );
  }
}
