...
export async function collectOptions(projectName: string, targetDir: string, flags: CliOptions): Promise<StackOptions> {
  ...
  return { projectName, targetDir, preset, apps, frontend, backend, ui, database, cache, auth, crud, ci, packageManager, docker, git, install, force: flags.force ?? false, debug: flags.debug ?? false, goModule };
}
