...
async function pkg(root: string, name: string, files: Record<string, string>): Promise<void> {
  await json(root, `packages/${name}/package.json`, { name: `@stackforge/${name}`, version: '0.1.0', private: true, type: 'module', exports: './src/index.ts' });
  await Promise.all(Object.entries(files).map(([file, content]) => write(root, `packages/${name}/${file}`, content)));
}
