...
export async function generateRoot(options: StackOptions): Promise<void> {
  ...
  for (const app of apps) {
    ...
    scripts[`dev:${app}`] = pm === 'npm' ? `npm run dev -w @stackbuild/${app}` : pm === 'yarn' ? `yarn workspace @stackbuild/${app} dev` : pm === 'bun' ? `bun --filter './apps/${app}' run dev` : `pnpm --filter @stackbuild/${app} dev`;
    scripts[`build:${app}`] = pm === 'npm' ? `npm run build -w @stackbuild/${app}` : pm === 'yarn' ? `yarn workspace @stackbuild/${app} build` : pm === 'bun' ? `bun --filter './apps/${app}' run build` : `pnpm --filter @stackbuild/${app} build`;
    scripts[`lint:${app}`] = pm === 'npm' ? `npm run lint -w @stackbuild/${app}` : pm === 'yarn' ? `yarn workspace @stackbuild/${app} lint` : pm === 'bun' ? `bun --filter './apps/${app}' run lint` : `pnpm --filter @stackbuild/${app} lint`;
  }
