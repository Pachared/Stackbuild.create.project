...
async function vite(o: StackOptions, app: Exclude<AppName, 'api'>): Promise<void> {
  ...
  await json(root, `${base}/package.json`, { name: `@stackforge/${app}`, private: true, version: '0.1.0', type: 'module', scripts: { dev: `vite --port ${VITE_PORTS[app]}`, build: 'tsc -b && vite build', lint: 'eslint .', typecheck: 'tsc -b --noEmit' }, dependencies: viteDeps(o), devDependencies: { '@vitejs/plugin-react': '^4.4.0', typescript: '^5.8.0', vite: '^6.3.0', eslint: '^9.0.0' } });
...
async function next(o: StackOptions, app: Exclude<AppName, 'api'>): Promise<void> {
  const root = o.targetDir, base = `apps/${app}`;
  await json(root, `${base}/package.json`, { name: `@stackforge/${app}`, private: true, version: '0.1.0', scripts: { dev: `next dev --port ${NEXT_PORTS[app]}`, build: 'next build', start: `next start --port ${NEXT_PORTS[app]}`, lint: 'eslint .', typecheck: 'tsc --noEmit' }, dependencies: nextDeps(o), devDependencies: { '@types/node': '^22.0.0', '@types/react': '^19.0.0', typescript: '^5.8.0', eslint: '^9.0.0' } });
